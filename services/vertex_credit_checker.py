...
class VertexCreditChecker:
    ...
    def format_pricing_info() -> str:
        ...
        text += f"   • New GCP accounts only\n"
        text += f"   • Applies to all GCP services\n\n"
    ...
    def get_account_info_text(project_id: str, location: str = "us-central1") -> str:
        ...
        text += "🔗 Quick Links:\n"
        text += f"   • Billing: {VertexCreditChecker.get_billing_console_url(project_id)}\n"
        text += f"   • Vertex AI: {VertexCreditChecker.get_vertex_ai_console_url(project_id, location)}\n"
        text += f"   • Quotas: {VertexCreditChecker.get_quotas_console_url(project_id)}\n\n"
